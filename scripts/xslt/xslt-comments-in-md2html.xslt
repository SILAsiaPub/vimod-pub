<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:my="my:my" exclude-result-prefixes="xml xsl xs my">
      ...
      <xsl:function name="my:parse-lines" as="element()*">
            ...
            <xsl:sequence select="my:parse-line($pLines, 1, count($pLines))"/>
      </xsl:function>
            ...
            <xsl:if test="not($pLineNum gt $pTotalLines)">
                  ...
                  <xsl:choose>
                        <xsl:when test="starts-with($vLine, '#')  and  ends-with($vLine, '#')  ">
                              ...
                              <xsl:sequence select="my:parse-line($pLines, $pLineNum+1, $pTotalLines)"/>
                        </xsl:when>
