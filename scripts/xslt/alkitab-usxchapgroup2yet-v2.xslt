...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="myfunctions">
      ...
      <xsl:variable name="section-or-parallel-scr-para-list" select="concat($non-scr-para-list-additions,' ',$section-para-list,' ',$parallel-para-list)"/>
...
"/>
      <xsl:template match="/">
            ...
            <xsl:apply-templates select="//para[@style = $section-or-parallel-scr-para-array]" mode="section"/>
      ...
      <xsl:template match="para" mode="section">
            ...
            <xsl:choose>
                  ...
                  <xsl:otherwise>
                        <xsl:text>&#10;XXXX-unhandled-section/ref@style=</xsl:text>
...
<xsl:if test="$includefootnote = $truearray">
      ...
      <xsl:template match="char[@style = 'xt']" mode="xref">
            ...
            <!--<xsl:variable name="fixedstringessentials">
                  <xsl:variable name="primaryseparator" select="'\r\n'"/>
                  <xsl:variable name="secondaryseparator" select="'='"/>
                  <xsl:variable name="wholearray" select="tokenize($xrefessentialreplacefixes,$primaryseparator)"/>
      ...
      <xsl:function name="f:rrefnumb">
            <!-- function to match USX book code to Alkitab numbering schema for r ref-->
            <xsl:param name="string"/>
            <xsl:variable name="toc">
                  <xsl:choose>
                        <xsl:when test="$rrefnamesmatch = 'toc3'">
                              <xsl:value-of select="number(4)"/>
                        </xsl:when>
                        <xsl:when test="$rrefnamesmatch = 'toc1'">
                              <xsl:value-of select="number(6)"/>
                        </xsl:when>
                        <xsl:otherwise>
                              <xsl:value-of select="number(3)"/>
      ...
      <xsl:template name="r-ref-parser">
            <xsl:param name="string"/>
            <xsl:variable name="add-book" select="translate(replace($string,'(\d? ?[A-Z][a-z\.\- ]+ )(\d+:[\d\-]+); (\d+):','$1$2; $1 $3'),'()','')"/>
            <xsl:variable name="part" select="tokenize($add-book,'; |, ')"/>
            <xsl:for-each select="$part">
                  <xsl:text>&#10;parallel&#9;</xsl:text>
                  <xsl:value-of select="."/>
            </xsl:for-each>
