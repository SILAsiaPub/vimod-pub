...
<!--
    #############################################################
    # Name:         compare-lists.xslt
    # Purpose:	compare two lists and return common entries
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="myfunctions">
      ...
      <xsl:param name="mainmatchcolumn" select="1"/>
      ...
      <xsl:template match="/">
            <xsl:for-each select="$full">
                  <xsl:variable name="source" select="tokenize(.,'&#9;')[$mainmatchcolumn]"/>
                  <xsl:variable name="lookupval" select="f:lookupdefault($country,$source,'&#9;',1,2,'')"/>
