...
<!--
    #############################################################
    # Name:         	bible-conc-sort-group2.xslt
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
      ...
      <xsl:template match="/*">
            <groupedWords>
                  <xsl:for-each-group select="w" group-by="lower-case(substring(@word,1,1))">
                        <xsl:sort select="substring(lower-case(@word),1,1)"/>
                        <xsl:element name="alphaGroup">
                              <xsl:attribute name="alpha">
                                    <xsl:value-of select="substring(lower-case(@word),1,1)"/>
                              </xsl:attribute>
                              <xsl:for-each-group select="current-group()" group-by="lower-case(@word)">
