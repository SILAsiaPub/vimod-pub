<!--
    #############################################################
    # Name:   	sfm2xml-group-level1.xslt
    # Purpose:	parses a sfm dictionary file with tables and outputs a grouped xml.
...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:f="myfunctions">
      ...
      <xsl:param name="group-node1_list"/>
        <xsl:param name="remove-node_list"/> 
      ...
      <xsl:variable name="group-nodes1" select="tokenize($group-node1_list,'\s+')"/>
        <xsl:variable name="remove-node" select="tokenize($remove-node_list,'\s+')"/> 
            ...
            <xsl:analyze-string select="replace(unparsed-text($sourcetexturi),'(\r)',' $1')" regex="\n\\">
                  ...
                  <xsl:non-matching-substring>
                        <xsl:choose>
                              ...
                              <xsl:otherwise>
                                    <xsl:element name="{replace(translate(tokenize(.,'\s')[1],'?+-\','xxx'),'^(\d)','z$1')}">
      ...
      <xsl:template match="/">
            <xsl:element name="{$rootnode}">
                  <xsl:for-each-group select="$flatxml/node()" group-starting-with="*[local-name() = $group-nodes1]">
...
<!--<xsl:template match="*[local-name() = $remove-node]">
      ...
      <xsl:template name="parseline">
            ...
            <xsl:for-each select="$cell">
                  ...
                  <xsl:if test="string-length(.) > 0">
                        <!-- test if there is content for the cell -->
